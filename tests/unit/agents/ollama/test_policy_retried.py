...
def test_policy_decided_event_captures_thought_in_transcript() -> None:
    definition = Connect4GameDefinition
    config = Connect4Config(rows=4, columns=4, connect_length=4)

    retry_sink: dict[int, list[tuple[int, str]]] = {0: [], 1: []}
    decision_sink: dict[int, list[tuple[int, str]]] = {0: [], 1: []}

    class _ThoughtClient:
        ...
        def chat(
            ...
            self._turn += 1
            content = json.dumps(
                {"thought": f"{self._label} picks {col}", "column": col}
            )
            return {"message": {"content": content}}

    agent0 = OllamaAgent(
        client=_ThoughtClient("A"),
        model="model-a",
        prompt_builder=Connect4PromptBuilder(),
        decision_callback=lambda a, t: decision_sink[0].append((a, t)),
    )
    agent1 = OllamaAgent(
        client=_ThoughtClient("B"),
        model="model-b",
        prompt_builder=Connect4PromptBuilder(),
        decision_callback=lambda a, t: decision_sink[1].append((a, t)),
    )
    ...
    decided = [e for e in transcript["events"] if e["event_type"] == "PolicyDecided"]
    assert len(decided) >= 2
    first = decided[0]
    assert first["event_scope"] == "runtime"
    assert first["payload"]["seat"] == 0
    assert first["payload"]["attempt"] == 1
    assert first["payload"]["thought"].startswith("A picks ")
    second = decided[1]
    assert second["payload"]["seat"] == 1
    assert second["payload"]["thought"].startswith("B picks ")
