...
>
> **Current status (verified 2026-09-24): Phases 0-37 complete.** Phase 37 shipped the chance-node primitive on wire `schema_version=2`. Chance is nature's action (`sample_chance` / `apply_chance`), the seed lives only on `LocalMatch.rng`, transcripts record outcomes and replay applies them without the seed, and **Pig** (`arena.games.pig`) is the exemplar game. 821 tests pass. **Next up: Phase 38, the imperfect-information contract, which carries the bump to `schema_version=3`.**
>
