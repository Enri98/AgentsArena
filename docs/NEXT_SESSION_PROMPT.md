...
>
> **Current status (verified 2026-09-25): Phases 0-41 complete.** Phase 38 moved the wire to `schema_version=3`. Every payload is now built per recipient: a viewer is a seat, or `None` for the public. Snapshots, chance outcomes, events (`DomainEvent.visible_to`), transcripts (`view`: full/seat/public), and welcome config are all redacted per viewer, and a reconnect replays the seat's own history. The shared contract proves redaction by indistinguishability (`PrivateVariant`). 1200+ tests pass. **Phases 39 (Liar's Dice) and 40 (public transcript persistence, `GET /matches/{id}/public-transcript`) are also complete; and Phase 41 (simultaneous moves, wire v4, Rock-Paper-Scissors) too; next up: Phase 42, docs + scaffold templates + TypeScript SDK.**
>
...
>
> **Next direction — v2 roadmap, Phases 36-42, approved 2026-09-22.** Specified in `IMPLEMENTATION_PLAN.md` under "v2 roadmap". Order: ~~36 spectator endpoint + transport refactor + §13 rate limits + CI~~ **COMPLETE 2026-09-23** → ~~37 chance-node primitive (bump to `schema_version=2`)~~ **COMPLETE 2026-09-24** → ~~38 imperfect-information contract (bump to 3)~~ **COMPLETE 2026-09-24** → ~~39 Liar's Dice~~ **COMPLETE 2026-09-24** → ~~40 transcript persistence + `GET /matches/{id}/public-transcript`~~ **COMPLETE 2026-09-25** → ~~41 generalized turn loop (bump to 4)~~ **COMPLETE 2026-09-25** → 42 docs + packaged TypeScript SDK. Expand only the current phase, one slice at a time.
>
